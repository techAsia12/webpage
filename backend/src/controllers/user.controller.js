...
const register = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    console.log("Inserting user into 'users' table: ", { phoneno, name, email, role });
    await dbQuery("INSERT INTO users (phoneno, name, password, email, role) VALUES (?, ?, ?, ?, ?)", [phoneno, name, hashPassword, email, role]);
    console.log("User inserted into 'users' table successfully");

    // Insert into 'client_dets'
    console.log("Inserting client details into 'client_dets' table for phoneno: ", phoneno);
    await dbQuery("INSERT INTO client_dets (phoneno, state, MACadd) VALUES (?, ?, ?)", [phoneno, state, null]);
    console.log("Client details inserted into 'client_dets' successfully");

    return res.status(200).json(new ApiResponse(200, { phoneno, email, name }, "User Registered Successfully"));
  } catch (err) {
...
const googleLogin = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (user.length > 0) {
      ...
      res.cookie("authToken", token, options);
      return res.status(200).json(new ApiResponse(200, { email, name, phoneno, role, token }, "Successfully logged In"));
    } else {
      await dbQuery("INSERT INTO users (phoneno, name, email) VALUES (?, ?, ?)", [0, name, email]);
      return res.status(201).json(new ApiResponse(201, { email, name }, "Successfully registered add phone no"));
    }
  } catch (error) {
...
const addPhoneno = asyncHandler(async (req, res, next) => {
  ...
  try {
    await dbQuery("UPDATE users SET phoneno = ?, role = ? WHERE email = ?", [phone, role, email]);

    if (role === "Client") {
      await dbQuery("INSERT INTO client_dets (phoneno, state) VALUES (?, ?)", [phone, state]);
    }

    const token = generateToken({ id: phone, email });
    res.cookie("authToken", token, options);

    return res.status(200).json(new ApiResponse(200, { user: { id: phone, email }, token }, "Registered successfully"));
  } catch (error) {
    return next(new ApiError(500, "Database Error"));
  }
});
...
const update = asyncHandler(async (req, res, next) => {
  ...
  try {
    const result = await dbQuery("UPDATE users SET name=?, email=? WHERE phoneno=?", [name, email, user.id]);
    
    return res.status(200).json(new ApiResponse(200, result[0], "Updated Successfully"));
  } catch (err) {
    return next(new ApiError(500, "Database Error"));
  }
});
...
const retiveHourlyUsage = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    result.forEach((entry) => {
      ...
      console.log(`For time: ${entry.time}, updating hour ${hour} with unit: ${entry.unit}`);
      unitsPerHour[hour] = entry.unit;
    });

    console.log("Units per hour:", unitsPerHour);

    return res.status(200).json(new ApiResponse(200, unitsPerHour, "Data Sent"));
...
const retiveWeeklyUsage = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    result.forEach((entry) => {
      ...
      console.log(`For time: ${entry.time}, updating day ${day} with unit: ${entry.unit}`);
...
const retiveYearlyUsage = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    result.forEach((entry) => {
      ...
      console.log(`For time: ${entry.time}, updating month ${month} with unit: ${entry.unit}`);
      unitsPerMonth[month] = entry.unit;
    });

    console.log("Units per month:", unitsPerMonth);

    return res.status(200).json(new ApiResponse(200, unitsPerMonth, "Data Sent"));
...
const getUserData = asyncHandler(async (req, res, next) => {
  try {
    const [result] = await db.promise().query(
      'SELECT name, email, phoneno, role FROM users WHERE phoneno = ?',
      [req.user.id]
    );
